...
{
	int clen=0,ll=1;
	while (left+ll+1<right-ll)
	{
		clen++;
		ll<<=1;
	}
	return min(st[left][clen],st[right-(1<<clen)+1][clen]);
}*/
...
int main ()
{
	...
	/*s[len+1]='$';
	for (i=len+2;i<=len+2+len-1;i++) s[i]=s[i-(i-len-1)*2];
	len=strlen(s+1);
	construct_sa();
	construct_lcp();
	init_st();
	for (i=1;i<=len;i++)
		if (sa[i]>nlen+1)
			for (j=i-1;j>=1 && !aft[j];j--)
				aft[j]=i;
	for (i=1;i<=len;i++) pos[sa[i]]=i;
	for (i=1;i<=nlen;i++)
	{
		int lft=pos[i];int rght=aft[lft];
		prelen[i]=query(lft,rght-1);
	}*/
	int cur=1;
	...
	//cout<<(gethash1(1,5)==gethash2(1,5))<<endl;
	//cout<<hsh1[5]<<' '<<hsh2[1]<<endl;
	return 0;
}