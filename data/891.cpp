...
{
  return (H0[R+1]-H0[L]*Pow[R-L+1]%Mod+Mod)%Mod;
}

long long Get1(int L,int R)
{
  return (H1[L]-H1[R+1]*Pow[R-L+1]%Mod+Mod)%Mod;
}
...
int main()
{
  ...
  H0[0]=0;for(int i=0;i<N;i++)H0[i+1]=H0[i]*Seed+S[i],H0[i+1]%=Mod;
  H1[N]=0;for(int i=N-1;i>=0;i--)H1[i]=H1[i+1]*Seed+S[i],H1[i]%=Mod;
  Pow[0]=1;for(int i=0;i<N;i++)Pow[i+1]=Pow[i]*Seed%Mod;
  ...
  for(int i=0;i<N;i++)
    {
      ...
      while(Left<Right)
        {
          int Mid=(Left+Right+1)/2;
//cout<<Left<<" "<<Right<<" "<<Get0(i-Mid,i)<<" "<<Get1(i,i+Mid)<<endl;
