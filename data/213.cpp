...
template<typename type1, typename type2> inline void _cutedebug(const pair<type1, type2> &a, ostream &cerr){
	cerr << "("; _cutedebug(a.fs, cerr); cerr << ", "; _cutedebug(a.sc, cerr); cerr << ")";
}
template<typename type> inline void _cutedebug(const vector<type> &a, ostream &cerr){
	cerr << "[";
	bool flag = true;
...
template<typename type1, typename type2> inline void _cutedebug(const map<type1, type2> &a, ostream &cerr){
	...
	for (typename map<type1, type2>::const_iterator i = a.begin(); i != a.end(); i++){
		if (flag)
			...
			cerr << ", ";
		cerr << "(";
		_cutedebug((*i).fs, cerr);
		cerr << ": ";
		_cutedebug((*i).sc, cerr);
		cerr << ")";
	}
	cerr << "}";
}
#ifdef CUTEBMAING
template<typename type> inline void _cutedebug_(const type &a, string name, int ln, ostream &cerr){
	cerr << "Line " << ln << " : " << name << " = "; _cutedebug(a, cerr); cerr << ";" << endl;
}
#define debug(x) _cutedebug_(x, #x, __LINE__, cerr)
#endif
#ifndef CUTEBMAING
#define debug(x) ({})
#endif
