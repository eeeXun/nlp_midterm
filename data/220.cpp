

/*
 *
 * 后缀数组的标准模板， 注意所有的下标全部从1开始
 *
 */

#include <iostream>
#include <cstring>
#include <algorithm>
#include <vector>
#include <map>
#include <set>

using namespace std;


const int MAX_N = 200005;       // 最长字符串长度


int P[MAX_N];       // P[i]表示以i位置为中心的最长回文子串半径
char __s[MAX_N];


// 返回最长回文子串长度
char ss[MAX_N * 2 + 5];
int manacher(char* s) {
    int n = strlen(s);
    if (n <= 1) {
        return n;
    }

    // 构造新字符串
    for (int i = 0; i < 2*n+1; i++) {
        if (i & 1) {
            ss[i] = s[i>>1];
        } else {
            ss[i] = '#';
        }
    }

    P[0] = 1;
    int L = 0, R = 0;       // 当前已经找到的所有回文子串中结束位置最靠右的回文子串的左右边界
    int max_r = 1;
    for (int i = 1; i < 2*n+1; i++) {
        if (i > R) {
            int ll = i, rr = i;
            while (ll-1 >= 0 && rr+1 < 2*n+1 && ss[ll-1] == ss[rr+1]) {
                ll--, rr++;
            }
            P[i] = rr - i + 1;
        } else {
            int j = L + R - i;
            int l_bound = j - P[j] + 1;
            if (l_bound > L) {
                P[i] = P[j];
            } else if (l_bound < L) {
                P[i] = R - i + 1;
            } else {
                int ll = i-P[j]+1, rr = i+P[j]-1;
                while (ll-1 >= 0 && rr+1 < 2*n+1 && ss[ll-1] == ss[rr+1]) {
                    ll--, rr++;
                }
                P[i] = rr - i + 1;
            }
        }

        if (i + P[i] - 1 > R) {
            R = i + P[i] - 1;
            L = i - P[i] + 1;
        }

        max_r = max(max_r, P[i]);
    }

    return max_r - 1;
}

// 求取前缀函数数组, pi[i]表示以i位置结束的前缀的前缀函数，实际上就是以i位置结束的前缀的最长公共前后缀长度，就是D[i]
int pi[MAX_N];          // pi[i]表示以i位置结束的前缀的前缀函数，实际上就是以i位置结束的前缀的最长公共前后缀长度，就是D[i]
int* get_kmp_pi(char* s, int n) {
    memset(pi, 0, sizeof(int) * n);
    pi[0] = 0;
    int i = 1, j = 0;
    while (i < n) {
        if (s[i] == s[j]) {
            pi[i] = j + 1;
            i++, j++;
        } else {
            if (j == 0) {
                i++;                // 最后一个可能配上的位置也失配了，D[i]就保持默认值0
            } else {
                j = pi[j-1];         // 模式串向后移动
            }
        }
    }

    return pi;
}

#define DEBUG_INPUT freopen("/Users/grh/Programming/CLionProjects/algo/input.txt", "r", stdin)
char buf[MAX_N];
int p_len[MAX_N];       // p_len记录以i位置为中心的最长回文的半径长度
int match[MAX_N];       // match[i]表示以i位置结束和以i以前的位置结束的所有前缀的最长公共前后缀长度的最大值
int match_pos[MAX_N];   // 记录最长公共前后缀长度最长的是哪个前缀


char buf1[MAX_N];

struct node {
    int a, b;
};


int main() {

    //DEBUG_INPUT;

    scanf("%s", buf);
    int n = strlen(buf);
    manacher(buf);

    for (int i = 0; i < n; i++) {
        p_len[i] = ((P[i*2 + 1] - 1) / 2) + 1;
    }

    int idx = 0;
    for (int i = n-1; i >= 0; i--) {
        buf1[idx++] = buf[i];
    }
    buf1[idx++] = '?';
    strcpy(buf1+idx, buf);
    get_kmp_pi(buf1, 2*n+1);

    for (int i = n+1; i < 2*n+1; i++) {
        match[i - (n+1)] = pi[i];
    }

    match_pos[0] = 0;
    for (int i = 1; i < n; i++) {
        if (match[i] > match[i-1]) {
            match_pos[i] = i;
        } else {
            match[i] = match[i-1];
            match_pos[i] = match_pos[i-1];
        }

        match[i] = max(match[i], match[i-1]);
    }

    // 枚举middle这一段的中心
    int ll[2], rr[2], mm[2];
    int max_len = 0;
    for (int i = 0; i < n; i++) {
        int left_len = n-1 - (i + p_len[i] - 1);

        if (left_len == 0 || i - (p_len[i] - 1) == 0 || match[i - (p_len[i] - 1)-1] == 0) {
            // 只有中间一段的情况
            if (2 * p_len[i] - 1 > max_len) {
                max_len = 2 * p_len[i] - 1;
                ll[0] = ll[1] = -1;
                rr[0] = rr[1] = -1;
                mm[0] = i - (p_len[i]-1);
                mm[1] = i + (p_len[i]-1);
            }
        } else {
            int prefix_len = min(left_len, match[i - (p_len[i] - 1)-1]);

            int cur_len = 2 * p_len[i] - 1 + prefix_len * 2;
            if (cur_len > max_len) {
                max_len = cur_len;
                ll[0] = match_pos[i - (p_len[i] - 1)-1] - prefix_len + 1;
                ll[1] = match_pos[i - (p_len[i] - 1)-1];
                rr[0] = n-1 - prefix_len + 1;
                rr[1] = n-1;
                mm[0] = i - (p_len[i]-1);
                mm[1] = i + (p_len[i]-1);
            }
        }
    }

    vector<node> v;
    v.push_back({ll[0], ll[1]});
    v.push_back({mm[0], mm[1]});
    v.push_back({rr[0], rr[1]});

    vector<node> ans;
    for (auto& val : v) {
        if (val.a == -1) {
            continue;
        }

        ans.push_back({val.a, val.b});
    }

    printf("%lu\n", ans.size());
    for (int i = 0; i < ans.size(); i++) {
        printf("%d %d\n", ans[i].a+1, ans[i].b - ans[i].a + 1);
    }

    return 0;
}



