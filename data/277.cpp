...
//	cout<<F<<endl;
	...
	{
		Z[i]=zi[i+s.length()];
	//	cout<<i<<" "<<i+s.length()<<" "<<Z[i]<<" "<<zi[i+s.length()]<<endl;
	}
}
const int maxt=1<<18;
int dat[maxt][3];
...
pair<int,int> query0(int x,int l,int r,int tl,int tr)
{
	...
	if(tl<=l&&r<=tr)
	{
	//	cout<<x<<" "<<l<<" "<<r<<" "<<dat[x][0]<<" "<<dat[x][2]<<endl;
...
void upd(int x,int l,int r,int pos,int len,int endpos)
{
	...
	//	cout<<x<<" "<<l<<" "<<dat[x][0]<<" "<<dat[x][1]<<" "<<dat[x][2]<<endl;
		return;
	}
	int mid=(l+r)>>1;
...
int main()
{
	...
	//	cout<<pp.first<<" "<<pp.second<<" "<<tmp<<endl;
	//	cout<<i<<" "<<Z[i]<<endl;
