...
struct HASH{
	...
	void build(int t,string s)
	{
		f[t][0]=s[0]-'a'+1;
		for(int i=1;i<n;i++)
		{
			f[t][i]=f[t][i-1]*ch%mod+s[i]-'a'+1;
			f[t][i]%=mod;
		}
	}
...
int main()
{
	...
	for(int i=0;i<n;i++)
	{
		while(cnt<=len[i])h[cnt]=i+cnt-1,cnt++;
		cnt=max(len[i]+1,cnt);
	}
	while(cnt<=n)h[cnt++]=-1;
	for(int i=0;i<n;i++)
	{
		...
		if(j*2-1+l*2>mx)
		{
			...
			if(l)ans[0].pb(h[l]-l+1),ans[1].pb(l);
			ans[0].pb(L),ans[1].pb(j*2-1);
			if(l)ans[0].pb(n-l),ans[1].pb(l);
		}
	}
	cout<<ans[0].size()<<"\n";
	for(int i=0;i<ans[0].size();i++)
		cout<<ans[0][i]+1<<" "<<ans[1][i]<<"\n";
	return 0;
}