...
const int N=100005,M1=1e9+7,P1=3846961,M2=998244353,P2=3072127;
char s[N];
int f[N],g[N],fac1[N],fac2[N],v1[N],v2[N],v3[N],v4[N];
signed main(){
	scanf("%s",s+1);
	int n=strlen(s+1),id=0,Max=0;
	reverse(s+1,s+n+1);
	for (int i=1;i<=n;i++){
	...
	fac1[0]=fac2[0]=1;
	for (int i=1;i<=n;i++)fac1[i]=fac1[i-1]*P1%M1,fac2[i]=fac2[i-1]*P2%M2;
	for (int i=1;i<=n;i++)v1[i]=(v1[i-1]*P1+s[i])%M1,v2[i]=(v2[i-1]*P2+s[i])%M2;
	for (int i=n;i;i--)v3[i]=(v3[i+1]*P1+s[i])%M1,v4[i]=(v4[i+1]*P2+s[i])%M2;
	for (int i=1;i<=n;i++){
		int l=0,r=i/2;
		while (l<r){
			int mid=(l+r+1)/2;
	...
	Max=0;int ans1=0,ans2=0,ans3=0,ans4=0,ans5=0,ans6=0;
	for (int i=1;i<=n;i++){
		int l=0,r=i-f[i]-1;
		while (l<r){
			int mid=(l+r+1)/2;
