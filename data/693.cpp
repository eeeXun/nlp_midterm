...
using namespace std;
const int N = 2e5 + 30, INF = 1e9;

struct node{
	int val, l, pos, mid;
}ans;
int n, mid, d[N], z[N], pos[N], res, mx;
char s[N], tmp[N];
...
signed main(){
	...
	s[n + 1] = '#';
	for(int i = n + 2; i <= 2 * n + 1; ++i) s[i] = s[2 * (n + 1) - i];
	mid = n + 1; n = 2 * n + 1;
	for(int i = 1, pos = 0, r = 0; i < mid; ++i){
	...
	for(int i = mid - 2; i >= 1; --i) pos[i] = max(pos[i], pos[i + 1]);
	for(int i = 1, l, r, m; i < mid; ++i){
