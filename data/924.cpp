...
char s[length], s1[length];
int len;
int next[length], p[length], pidx[length], pld[length];

void GetNext(char s[], int next[]){//��ƥ����
    int l = strlen(s), i = 0, j = -1;
    next[0] = -1;
    while(i <= l) {//ע��������lʱ��ҲҪ����ƥ�䡣
        if(j == -1 || s[i] == s[j]){
             j ++ ; i ++ ;
             if(s[i] != s[j]) next[i] = j;
             else next[i] = next[j];
...
void Kmp(char s[], char s1[], int next[], int p[]){//��ƥ����,s1->s
