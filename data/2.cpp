...
int main() {
    ...
    // disallow even
    F0R (i, sz(s) - 1) pal.width[2 * i + 1] = 0;
    ...
    sparse_table<int, suffix_array<POS_INF>::smaller> inx_rmq(sz(str), sa.lcp_cmp,
            [&](int i) { return sa.by_rank[i]; });

    int x = -1, y = 0, p = -1;

    F0R (suff, (sz(s) - 1) / 2 + 1) {
        pair<int, int> pref_range = sa.ranks_with_prefix(sz(s) + 1, suff);
        int earliest = inx_rmq(pref_range.f, pref_range.s);

        if (earliest + 2 * suff <= sz(s)) {
            int pos = earliest + suff;
            int len = sz(s) - earliest - 2 * suff;
            int mid = pal.longest_within_substring(pos, len);

            /*pd(s.substr(pos, len), mid);
            FOR (i, pos, pos + len)
                pr(pal.width[2 * i], " ");
            ps();*/
