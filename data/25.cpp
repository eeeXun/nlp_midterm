...
ll ha1[100010],hr1[100010],mi1[100010];
ll ha2[100010],hr2[100010],mi2[100010];
ll getha1(int l,int r)
{
	ll rt=ha1[r];
	if(l>0)
		rt=(rt-ha1[l-1]*mi1[r-l+1]%md1+md1)%md1;
	return rt;
}
ll gethr1(int l,int r)
{
	ll rt=hr1[r];
	if(l>0)
		rt=(rt-hr1[l-1]*mi1[r-l+1]%md1+md1)%md1;
	return rt;
}
ll getha2(int l,int r)
{
	ll rt=ha2[r];
	if(l>0)
		rt=(rt-ha2[l-1]*mi2[r-l+1]%md2+md2)%md2;
	return rt;
}
ll gethr2(int l,int r)
{
	ll rt=hr2[r];
	if(l>0)
		rt=(rt-hr2[l-1]*mi2[r-l+1]%md2+md2)%md2;
	return rt;
}
...
int main()
{
	...
	for(int i=0;i<n;i++)
	{
		...
		{
			ha1[i]=(ha1[i-1]*se1+zf[i])%md1;
			hr1[i]=(hr1[i-1]*se1+zf[n-1-i])%md1;
		}
	}
	for(int i=0;i<n;i++)
	{
		...
		{
			ha2[i]=(ha2[i-1]*se2+zf[i])%md2;
			hr2[i]=(hr2[i-1]*se2+zf[n-1-i])%md2;
		}
	}
	mi1[0]=1;
	for(int i=1;i<=n;i++)
		mi1[i]=mi1[i-1]*se1%md1;
	mi2[0]=1;
	for(int i=1;i<=n;i++)
		mi2[i]=mi2[i-1]*se2%md2;
	manacher(zf,n,cd);
	yuclrmq(cd,n);
