...
{
	pi tp,tm,ts;
	password (pi TP=pi(0,0),pi TM=pi(0,0),pi TS=pi(0,0)){tp=TP;tm=TM;ts=TS;}
	bool operator < (const password &x)const{return tp.se+tm.se+ts.se<x.tp.se+x.tm.se+x.ts.se;}
}ans;
struct query
{
	int l,r,id;
...
void manacher()
{
	...
	for(int i=n,j=pc;i>=1;i--)
	{
		...
		password res(pi(resp.l-resp.w+1,resp.w),pi(x-pr[x]+1,2*pr[x]-1),pi(resp.r,resp.w));
		if(ans<res)ans=res;
	}
}
...
int main()
{
	...
	calc();
	manacher();
	printf("%d\n",(ans.tp.se>0)+1+(ans.ts.se>0));
	if(ans.tp.se)printf("%d %d\n",ans.tp.fi,ans.tp.se);
	printf("%d %d\n",ans.tm.fi,ans.tm.se);
	if(ans.ts.se)printf("%d %d\n",ans.ts.fi,ans.ts.se);
	return 0;
}