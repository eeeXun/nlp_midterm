...
int main()
{
	...
	//for (int i=1;i<=n*2+1;i++) printf("%c %d\n",b[i],r[i]);
	//ababa
	//1112
	...
	//for (int i=1;i<=n;i++) printf("%c",a1[i]);
	//printf("\n");
	//for (int i=1;i<=n;i++) printf("%d",s[i]);
	//printf("\n");
	rx=0; mx=0;
	for (int i=1;i<=n;i++)
	{//l+1=i*2 i=(l+1)/2 l%2==1
	//i*2-r[i*2]
		int x=(i*2-r[i*2]-1)/2;
		//printf("i=%d x=%d s2[x]=%d \n",i,x,s2[x]);
		if (x==0||i*2-x>n)
		{
			//printf("%d %d\n",x+1,i*2-x-1);
		...
		else
		{
			if (n-s2[x]+1>=i*2-x)
			{
				//printf("%d %d %d %d %d %d\n",bh[x]-s2[x]+1,bh[x],x+1,i*2-x-1,n-s2[x]+1,n);
				if (s2[x]*2+i*2-x-1-x-1+1>mx) rx=i,mx=s2[x]*2+i*2-x-1-x-1+1;
			}
			else
			{
				//printf("%d %d %d %d %d %d\n",i*2-x-n+bh[x],bh[x],x+1,i*2-x-1,i*2-x,n);
