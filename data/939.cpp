...
string y[2];
int v[N];
pair<int, int> tree[4 * N]; // first is the length, second is the index of the center

void manacher(string s) { // returns length and start index of longest palindrome 
    int n = s.size();
    s = "$" + s + "^";
    vector<int> p(n + 2);
    int l = 0, r = -1;
...
#define get(l, r) get(1, 0, n - 1, l, r)
...
void solve(int tc) {
    ...
    for (int i = n + 1; i < 2 * n + 1; i++) if (p[i] > 0 && i - n + 1 <= n - p[i]) {
        ...
        int L = 1, R = len;
        auto tmp = (l + len / 2 <= r - len / 2 ? get(l + len / 2, r - len / 2) : neg);
        ...
        if (tmp.first >= len)
            ...
            while (R - L > 2) {
                ...
                if ((m & 1) == 0) m--;
                if (m < L) break;
                if (l + m / 2 <= r - m / 2 && get(l + m / 2, r - m / 2).first >= m) L = m;
                else R = m;
            }
            maximize(L, get(l + L / 2, r - L / 2).second);
        }
    }
    print();
}
...
/*
Problem abstraction:
You are given a string s. Consider all two substrings (possibly empty) in s p_1 and p_2
so that p_1 is the reverse of p_2, and there exists a non-empty palindromic substring of odd length y in
between them. Of all such substrings, maximize |p_1| + y + |p_2|
We denote |p_1| = |p_2| = x
*/