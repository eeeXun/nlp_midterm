//ayame保佑，狗妈保佑，FBK保佑，天哥保佑，MDR保佑，锉刀怪保佑，M99保佑，克爹保佑
#include<bits/stdc++.h>
using namespace std;
int p1=1000000,p2=0;
...
int Hash(int l,int r,bool flg) {
	return flg?(h2[l]-h2[r+1]*pw[r-l+1]%mod+mod)%mod:(h1[r]-h1[l-1]*pw[r-l+1]%mod+mod)%mod;
}
...
int main() {
	pw[0]=1;
	for(int i=1; i<=100000; i++)pw[i]=pw[i-1]*mul%mod;
	c[0]='#';
	scanf("%s",c+1);
	manacher(c);
	n=strlen(c+1);
	for(int i=1; i<=n; i++)h1[i]=(h1[i-1]*mul+c[i])%mod;
	for(int i=n; i>=1; i--)h2[i]=(h2[i+1]*mul+c[i])%mod;
	for(int i=1; i<=n; i++) {
		int l=0,r=n-i+1;
		while(l<=r) {
			int mid=(l+r)>>1;
