...
void Manachar()
{
	int N=0,id=0,mx=0;rep(i,1,n) S[++N]='*',S[++N]=s[i];S[++N]='*';
	rep(i,1,N)
	{
	...
	rep(i,1,N) if (!(i&1)) far[i/2]=p[i]-1;
}
par get(int L,int R)
{
	int Lg=Log[R-L+1],x=Max[L][Lg],y=Max[R-(1<<Lg)+1][Lg];
	if (x>=y) return mp(x,Who[L][Lg]);
	else return mp(y,Who[R-(1<<Lg)+1][Lg]);
}
void Rmq()
{
	Log[0]=-1;rep(i,1,n) Log[i]=Log[i>>1]+1;
	rep(i,1,n) Max[i][0]=far[i],Who[i][0]=i;
...
par getmax(int L,int R)
{
	int l=0,r=(R-L)/2,mid;par res;
	while (l<=r)
	{
		mid=l+(r-l>>1);par tmp=get(L+mid,R-mid);
		if (tmp.fi<2*mid+1) r=mid-1;
		else res=mp(mid,tmp.se),l=mid+1;
	}
	return res;
}
int main()
{
	...
	par tmp=getmax(1,n);pp[1]=tmp.se-tmp.fi,ll[1]=2*tmp.fi+1;
	for (int R=n,L=0,j1=0,j2=0;;R--)
	{
		...
		if (L+1>R-1) break;par tmp=getmax(L+1,R-1);
		if (2*m+2*tmp.fi+1>ll[0]+ll[1]+ll[2])
		{
			pp[0]=L-m+1,ll[0]=m;
			pp[1]=tmp.se-tmp.fi,ll[1]=2*tmp.fi+1;
			pp[2]=R,ll[2]=m;
		}
	}
	if (!pp[0]) {printf("1\n");printf("%d %d\n",pp[1],ll[1]);}
