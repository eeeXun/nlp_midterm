...
void Z(){
 ...
 for(int i=n;i>=1;i--){
  ...
  if(pq.EMPTY()) pre[i]=make_pair(0,i);
  else pre[i]=make_pair(pq.TOP()-i+1,pq.TOP());
...
int main(){
 ...
 for(int i=1;i<=n;i++){
  ...
  pii ret=pre[R+1];ret.first=min(ret.first,L-1);
  ret.second=ret.second-ret.first+1;
  if(ret.first>0){
   if(2*mch[i]-1+2*ret.first>K){
    K=2*mch[i]-1+2*ret.first;
    num=3;
    ans[0]=ret.first,len[0]=ret.first;
    ans[1]=R,len[1]=2*mch[i]-1;
    ans[2]=ret.second+ret.first-1,len[2]=ret.first;
   }
  }else if(2*mch[i]-1>K){
   K=2*mch[i]-1;
   num=1;
