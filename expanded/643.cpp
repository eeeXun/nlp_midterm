...
void SuffixArray::suffixBuild(int* T, int* SA, int n, int K) {
 ...
 for(int i = 0, j = 0, k = n0-n1; i < n; i++) {
  ...
  int p = (SA12[k] < n0 ? SA12[k]*3+1 : (SA12[k]-n0)*3+2), t = SA0[j];
  if(SA12[k] < n0 ?
   ...
   SA[i] = t; j++;
   if(j != n0) continue;
   for(i++; i < n; i++, k++) SA[i] = (SA12[k] < n0 ? SA12[k]*3+1 : (SA12[k]-n0)*3+2);
  }
 }
...
SegTree::SegTree(int _x, int _y) : left(
# 126 "data/643.cpp" 3 4
                                       __null
# 126 "data/643.cpp"
                                           ), right(
# 126 "data/643.cpp" 3 4
