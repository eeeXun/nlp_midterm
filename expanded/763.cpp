...
template<class T, class T2> inline istream &operator>>(istream &is, pair<T, T2> &p) { is>>p.first>>p.second; return is; }
template<class T, class T2> inline ostream &operator<<(ostream &os, const pair<T, T2> &p) { os<<p.first<<' '<<p.second; return os; }
...
template<class T, class T2> inline ostream &operator<<(ostream &os, const map<T, T2> &m) { auto id = m.begin(); while (id != m.end()) os<<(*id).first<<' '<<(*(id++)).second; return os; }
template<class T, class T2> inline ostream &operator<<(ostream &os, const unordered_map<T, T2> &um) { auto id = um.begin(); while (id != um.end()) os<<(*id).first<<' '<<(*(id++)).second; return os; }
template<class T> inline ostream &operator<<(ostream &os, const list<T> &l) { auto id = l.begin(); while (id != l.end()) os<<*(id++)<<' '; return os; }
template<class T> inline ostream &operator<<(ostream &os, const set<T> &second) { auto id = second.begin(); while (id != second.end()) os<<*(id++)<<' '; return os; }
...
void preKMP() {
    ...
    for (int i=n-2; i>=0; i--) {
        int p = dp[i+1];
        while (p < n-1 && second[p] != second[i]) {
            p = dp[p+1];
        }
        if (second[p] == second[i]) {
            p--;
        }
        dp[i] = p;
    }
...
void KMP() {
    ...
    for (int i=0; i<n; i++) {
        while (p < n-1 && (second[p] != second[i] || i+n-p >= n)) {
            p = dp[p+1];
        }
        if (second[p] == second[i]) {
            p--;
...
void hashString() {
    ...
    hsh[0] = second[0] - 'a' + 1;
    for (int i=1; i<n; i++) {
        hsh[i] = ((second[i]-'a'+1)*ppow[i] + hsh[i-1]) % mod;
    }

    rhsh.resize(n);
    rhsh[n-1] = second[n-1] - 'a' + 1;
    for (int i=n-2; i>=0; i--) {
        rhsh[i] = ((second[i]-'a'+1)*ppow[n-1-i] + rhsh[i+1]) % mod;
    }
}
