...
char s[100010*2];int n,a[100010*2],b[100010],c[100010],p[100010],f[100010*4],g[100010*4],S=0,T,L1,R1,L2,R2,L3,R3;
...
void upd(int p,int l,int r,int L,int R)
{
    ...
    upd(p*2,g[p]),upd(p*2+1,g[p]),g[p]=1000000007;
    int m=(l+r)/2;
...
int main()
{
    ...
    for(int i=1;i<=n*2;i++)a[i]=a[i-1]*1000000007 +s[i];
    p[0]=1;for(int i=1;i<=n;i++)p[i]=p[i-1]*1000000007;
    for(int i=1;i<=n*4;i++)f[i]=g[i]=1000000007;
    for(int i=1;i<=n;i++)
    {
        int L=1,R=n-i+1;
        while(L<=R)
