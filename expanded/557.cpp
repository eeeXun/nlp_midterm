...
{
 if (s<e) return hl[e]-hl[s]*pow[e-s];else return hr[e]-hr[s]*pow[s-e];
}
int calc(int x)
{
...
int main()
{
    ...
    for (i=1;i<=n;i++)
    {
        ...
        {
              mid=s+e-(s+e)/2;
              if (hash(i,i+mid-1)==hash(i,i-mid+1)) s=mid; else e=mid-1;
        }
        f[i]=s;
    }
    s=1;
    ...
    for (i=1;i<=n;i++)
    {
        ...
        if(s>e)g[i]=0;else g[i]=i-dl[s]+1;
    }
    for (i=1;i<=n;i++)
    {
        m[i]=m[i-1];
    ...
    for (i=1;i<=n;i++)
        ans=max(ans,calc(i));
    for (i=1;i<=n;i++)if (ans==calc(i))
 {
  s=i-f[i]+1;
        e=i+f[i]-1;
        j=min(g[m[s-1]],n-e);
        k=m[s-1]-g[m[s-1]]+1;
        if(j)cout<<3<<endl<<k<<" "<<j<<endl<<s<<" "<<e-s+1<<endl<<n-j+1<<" "<<j<<endl;else cout<<1<<endl<<s<<" "<<e-s+1<<endl;
  return 0;
 }
 return 0;
}
