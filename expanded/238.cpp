...
namespace RMQ {
  ...
  void build(int *A) {
    for (int i = (1), iend = (n); i <= iend; ++i) pos[0][i] = i;
    for (int i = (2), iend = (n); i <= iend; ++i) lg[i] = lg[i >> 1] + 1;
    for (int i = (1), iend = (16); i <= iend; ++i) for (int j = (1), jend = (n - (1 << i) + 1); j <= jend; ++j) {
...
void manacher() {
  ...
  int len = n + n + 1;
  str[1] = '#', str[0] = '!';
  for (int i = (1), iend = (n); i <= iend; ++i) str[i * 2] = A[i], str[i * 2 + 1] = '#';
  int pos = 0, mx = 0;
  for (int i = (1), iend = (len); i <= iend; ++i) {
    if (i <= mx) ext[i] = min(ext[2 * pos - i], mx - i);
    while (str[i - ext[i]] == str[i + ext[i]]) ext[i]++;
    if (i + ext[i] > mx) mx = i + ext[i], pos = i;
  }
  for (int i = (1), iend = (n); i <= iend; ++i) ext[i] = ext[i * 2] / 2;
}
...
void solve() {
  ...
  for (int i = (1), iend = (n); i <= iend; ++i) if (app[i]) {
