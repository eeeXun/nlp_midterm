...
VI manacher(const string& s) {
    ...
    for(int i=int(0); i<=int((static_cast<int>((s).size()))-1); ++i) {
        ...
        while(i-cur-1>=0 && i+cur+1<static_cast<int>((s).size()) && s[i-cur-1]==s[i+cur+1]) {
            ++cur;
        }
        ans[i]=cur;
        if(setmax(r, i+cur)) {
...
VI kmp(const string& s) {
    ...
    VI fail(static_cast<int>((t).size()));
    int j=fail[0]=-1;
    for(int i=int(1); i<=int((static_cast<int>((t).size()))-1); ++i) {
...
int main() {
    ...
    for(int p=int(0); p<=int((n)-1); ++p) {
        const int w=palin[p];
        int len=mx[p-w].first, i=mx[p-w].second-len+1;
        setmin(len, n-(p+w+1));
        if(len>0) {
            setmax(ans, make_pair(w*2+1+len*2, VPI{{i,i+len}, {p-w,p+w+1}, {n-len,n}}));
        } else {
            setmax(ans, make_pair(w*2+1, VPI{{p-w,p+w+1}}));
        }
    }
