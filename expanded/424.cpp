...
{int num=0;if(k<0)putchar('-'),k=-k;do{outputarray[++num]=k%10,k/=10;}while(k);while(num)putchar(outputarray[num--]+'0');}
...
int main()
{
 ...
 {
  puts("1");
  write(ps-pa[ps]+1),putchar(' '),write(pa[ps]*2-1),puts("");
  return 0;
 }
 puts("3");
 write(l[num]),putchar(' '),write(n-num+1),puts("");
 for(int i=l[num]+n-num+1;i<num;i++)if(pa[i]>=len&&i-len+1>l[num]+n-num&&i+len-1<num)pos=i;
 write(pos-len+1),putchar(' '),write(2*len-1),puts("");
 write(num),putchar(' '),write(n-num+1),puts("");
 return 0;
}
