...
const pii second={2333,9973};
pii operator + (const pii & a,const pii & b)
{
 return pii((a.first+b.first)%mo[0],(a.second+b.second)%mo[1]);
}
pii operator - (const pii & a,const pii & b)
{
 return pii((a.first-b.first+mo[0])%mo[0],(a.second-b.second+mo[1])%mo[1]);
}
pii operator * (const pii & a,const pii & b)
{
 return pii((ll)a.first*b.first%mo[0],(ll)a.second*b.second%mo[1]);
}
...
int main()
{
 ...
 powse[0] = pii(1,1);
 for(int i=1;i<=n;i++)
  powse[i] = powse[i-1] * second;

 hs[0] = pii(0,0);
 for(int i=1;i<=n;i++)
  hs[i] = hs[i-1] * second + pii(S[i],S[i]);
 rs[0] = pii(0,0);
 for(int i=1;i<=n;i++)
  rs[i] = rs[i-1] * second + pii(S[n-i+1],S[n-i+1]);
 ...
 if(ansf[0].first+ansf[0].second == ansf[1].first && ansf[1].first+ansf[1].second == ansf[2].first) {
  ansf[0] = pii(ansf[0].first,ansf[0].second+ansf[1].second+ansf[2].second);
  ansf[1] = ansf[2] = pii(-1,-1);
 }
 if(ansf[1] == pii(-1,-1)) {
  cout << 1 << endl;
  cout << ansf[0].first << ' ' << ansf[0].second << endl;
 } else {
  cout << 3 << endl;
  for(int k=0;k<3;k++)
   cout << ansf[k].first << ' ' << ansf[k].second << endl;
 }
 return 0;
}
