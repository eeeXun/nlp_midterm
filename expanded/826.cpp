...
long long h[200000], rh[200000], b[200000], L, a[200000];
string s;
...
# 17 "data/826.cpp"
    ...
    for (long long i = 0; i < s.length(); i++){
        long long l = 1, r = min(i + 1, s.length() - i) + 1;
        while(r - l > 1){
            int mid = (r + l) / 2;
        ...
        if (L < 2 * l + 2 * d - 1){
            L = 2 * l + 2 * d - 1;
            ans.clear();
            ans.push_back({b[l] + 1, l});
            ans.push_back({i + 2 - d, 2 * d - 1});
            ans.push_back({s.length() - l + 1, l});
        }
    }
    int t1 = 0;
    int t2 = 0;
    int t3 = 0;
    if (ans[0].second == 0) t1 = 1;
    if (ans[1].second == 0) t2 = 1;
    if (ans[2].second == 0) t3 = 1;
    {
